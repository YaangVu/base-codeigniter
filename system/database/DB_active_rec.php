...
class CI_DB_active_record extends CI_DB_driver {
    ...
    /**
     *  FETCH
     * @param	integer	the FETCH NEXT value
     * @return	object
     */
    public function fetch($fetch = 10) {
        $this->ar_fetch = (int) $fetch;
        return $this;
    }
    ...
    protected function _compile_select($select_override = FALSE) {
        ...
        if (is_numeric($this->ar_offset) && is_numeric($this->ar_fetch) && count($this->ar_orderby) > 0) {
            $sql .= "\n";
            $sql .= "OFFSET " . $this->ar_offset . ' ROWS';
            $sql .= "\n";
            $sql .= "FETCH NEXT " . $this->ar_fetch . ' ROWS ONLY';
        }
